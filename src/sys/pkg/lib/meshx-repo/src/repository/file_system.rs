...
use {
    crate::delivery_blob::DeliveryBlobType,
    crate::{
    ...
    tuf::{
        ...
        repository::{
            ...
            FileSystemRepositoryBuilder as TufFileSystemRepositoryBuilder, RepositoryProvider as TufRepositoryProvider,
        ...
        async move {
            ...
            let content_range = match range {
                ...
                Range::Inclusive {
                    ...
                    if first_byte_pos > last_byte_pos || first_byte_pos >= total_len || last_byte_pos >= total_len {
    ...
    fn store_blob<'a>(&'a self, hash: &Hash, len: u64, src: &Utf8Path) -> BoxFuture<'a, Result<()>> {
        ...
        async move {
            ...
            match self.copy_mode {
                ...
                CopyMode::HardLink => {
                    let is_hardlink = if let Some(dst_metadata) = &dst_metadata {
                        src_metadata.dev() == dst_metadata.dev() && src_metadata.ino() == dst_metadata.ino()
                    } else {
                        false
                    };
            ...
            if let Some(blob_type) = self.delivery_blob_type {
                let dst = sanitize_path(&self.blob_repo_path, &format!("{}/{hash_str}", blob_type as u32))?;
...
async fn generate_delivery_blob(src: &Utf8Path, dst: &Utf8Path, blob_type: DeliveryBlobType) -> Result<()> {
    let src_blob = fs::read(src).with_context(|| format!("reading {src}"))?;

    let temp_path = create_temp_file(dst).await?;
    let file = std::fs::File::create(&temp_path)?;
    
    crate::delivery_blob::generate_to(blob_type, &src_blob, std::io::BufWriter::new(file)).context("generate delivery blob")?;
...
mod tests {
    ...
    async fn test_store_blob_verifies_src_length() {
        ...
        let err = repo
            .store_blob(&hash, contents.len() as u64 + 1, &path)
            .await
            .unwrap_err();
    ...
    async fn test_store_blob_copy_breaks_hardlinks() {
        ...
        let repo = FileSystemRepository::builder(metadata_repo_path.clone(), blob_repo_path.clone())
