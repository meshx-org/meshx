...
use crate::{MetaContents, MetaContentsError, MetaPackage, MetaPackageError, MetaSubpackages, MetaSubpackagesError};
...
impl PackageDirectory {
    ...
    pub async fn open_file(&self, path: &str, rights: OpenRights) -> Result<fio::FileProxy, OpenError> {
    ...
    pub async fn meta_contents(&self) -> Result<MetaContents, LoadMetaContentsError> {
        let meta_contents = self
            .read_file_to_string("meta/contents")
            .await
            .map_err(LoadMetaContentsError::Read)?;
        let meta_contents =
            MetaContents::deserialize(meta_contents.as_bytes()).map_err(LoadMetaContentsError::Parse)?;
        Ok(meta_contents)
    }

    /// Reads and parses the package's meta/package file.
    pub async fn meta_package(&self) -> Result<MetaPackage, LoadMetaPackageError> {
        let meta_package = self
            .read_file_to_string("meta/package")
            .await
            .map_err(LoadMetaPackageError::Read)?;
        let meta_package = MetaPackage::deserialize(meta_package.as_bytes()).map_err(LoadMetaPackageError::Parse)?;
        Ok(meta_package)
    }
    ...
    pub async fn meta_subpackages(&self) -> Result<MetaSubpackages, LoadMetaSubpackagesError> {
        match self.read_file_to_string(MetaSubpackages::PATH).await {
            Ok(file) => Ok(MetaSubpackages::deserialize(file.as_bytes()).map_err(LoadMetaSubpackagesError::Parse)?),
            Err(ReadError::Open(OpenError::OpenError(zx_status::Status::NOT_FOUND))) => Ok(MetaSubpackages::default()),
    ...
    pub async fn abi_revision(&self) -> Result<AbiRevision, LoadAbiRevisionError> {
        let abi_revision_bytes =
            fuchsia_fs::file::read(&self.open_file(AbiRevision::PATH, OpenRights::Read).await?).await?;
...
impl OpenRights {
    fn to_flags(&self) -> fio::OpenFlags {
        match self {
            ...
            OpenRights::ReadExecute => fio::OpenFlags::RIGHT_READABLE | fio::OpenFlags::RIGHT_EXECUTABLE,
        }
    }
}
...
mod tests {
    ...
    async fn list_blobs() {
        ...
        let duplicate_blob_merkle = fuchsia_merkle::MerkleTree::from_reader("Hello World!".as_bytes())
            .unwrap()
            .root();
