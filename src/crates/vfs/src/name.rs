...
use {
    midl_meshx_io as mio, fiber_status::Status, static_assertions::const_assert_eq,
...
pub const MAX_NAME_LENGTH: usize = mio::MAX_NAME_LENGTH as usize;
const_assert_eq!(MAX_NAME_LENGTH as u64, mio::MAX_NAME_LENGTH);
...
mod tests {
    ...
    fn test_parse_name() {
        ...
        assert_matches!(
            parse_name(
                std::str::from_utf8(&vec![65; mio::MAX_FILENAME as usize + 1]).unwrap().to_string()
            ),
            Err(ParseNameError::TooLong(_))
        );
        assert_matches!(
            parse_name(
                std::str::from_utf8(&vec![65; mio::MAX_FILENAME as usize]).unwrap().to_string()
            ),
            Ok(_)
        );
    ...
    fn test_validate_name() {
        ...
        assert_matches!(
            validate_name(std::str::from_utf8(&vec![65; mio::MAX_FILENAME as usize + 1]).unwrap()),
            Err(ParseNameError::TooLong(_))
        );
        assert_matches!(
            validate_name(std::str::from_utf8(&vec![65; mio::MAX_FILENAME as usize]).unwrap()),
            Ok(())
        );
