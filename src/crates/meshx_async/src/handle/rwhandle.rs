...
const OBJECT_PEER_CLOSED: fx::Signals = fx::Signals::OBJECT_PEER_CLOSED;
const OBJECT_READABLE: fx::Signals = fx::Signals::OBJECT_READABLE;
const OBJECT_WRITABLE: fx::Signals = fx::Signals::OBJECT_WRITABLE;
...
pub trait ReadableHandle {
    ...
    fn poll_readable(&self, cx: &mut Context<'_>) -> Poll<Result<ReadableState, fx::Status>>;

    /// Arranges for the current task to be woken when the object receives an
    /// `OBJECT_READABLE` or `OBJECT_PEER_CLOSED` signal.
    fn need_readable(&self, cx: &mut Context<'_>) -> Result<(), fx::Status>;
}
...
pub trait WritableHandle {
    ...
    /// no-op waker before the first poll.
    fn poll_writable(&self, cx: &mut Context<'_>) -> Poll<Result<WritableState, fx::Status>>;

    /// Arranges for the current task to be woken when the object receives an
    /// `OBJECT_WRITABLE` or `OBJECT_PEER_CLOSED` signal.
    fn need_writable(&self, cx: &mut Context<'_>) -> Result<(), fx::Status>;
}
...
impl PacketReceiver for RWPacketReceiver {
    fn receive_packet(&self, packet: fx::Packet) {
        let new = if let fx::PacketContents::SignalOne(p) = packet.contents() {
            p.observed()
        } else {
            return;
        };
        let old = fx::Signals::from_bits_truncate(self.signals.fetch_or(new.bits(), Ordering::SeqCst));
    ...
    T: AsHandleRef,
{
    ...
    pub fn new(handle: T) -> Result<Self, fx::Status> {
    ...
    pub fn is_closed(&self) -> bool {
        let signals = fx::Signals::from_bits_truncate(self.receiver().signals.load(Ordering::Relaxed));
        ...
        match self.handle.wait_handle(OBJECT_PEER_CLOSED, fx::Time::INFINITE_PAST) {
            Ok(_) => true,
            Err(fx::Status::TIMED_OUT) => false,
            Err(status) => {
    ...
    fn need_signal(&self, cx: &mut Context<'_>, task: &AtomicWaker, signal: fx::Signals) -> Result<(), fx::Status> {
    ...
    fn schedule_packet(&self, signals: fx::Signals) -> Result<(), fx::Status> {
...
{
    fn poll_readable(&self, cx: &mut Context<'_>) -> Poll<Result<ReadableState, fx::Status>> {
        let signals = fx::Signals::from_bits_truncate(self.receiver().signals.load(Ordering::SeqCst));
