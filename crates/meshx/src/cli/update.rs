//! Command for self-updating the `meshx` CLI tool
...
const REPO: &str = "meshx-org/meshx";
const BINARY_NAME: &str = "meshx";
...
/// CLI command for updating meshx to the latest version
///
/// # Examples
///
...
/// ```bash
/// meshx update --git myorg/my-private-meshx --token ghp_xxxxxxxxxxxx
/// ```
///
/// Update from private repository using environment variable:
/// ```bash
/// export GITHUB_TOKEN=ghp_xxxxxxxxxxxx
/// meshx update --git myorg/my-private-meshx
/// ```
...
pub struct UpdateCommand {
    ...
    #[clap(long, env = "MESHX_GITHUB_TOKEN")]
    token: Option<String>,
...
fn parse_version(tag: &str) -> Option<Version> {
    let t = tag.strip_prefix("meshx-").unwrap_or(tag);
...
impl CliCommand for UpdateCommand {
    ...
    async fn handle(&self, ctx: &CliContext) -> anyhow::Result<CommandOutput> {
        ...
        {
            debug!("Current meshx version: {}", current);
        }
        ...
        if self.dry_run {
            ...
            return Ok(CommandOutput::ok(
                format!(
                    "Would update meshx from {current_version_str} to {tag_name}",
        ...
        let current_meshx = std::env::current_exe();
        let cache_dir = ctx.deref().in_cache_dir("update");
        tokio::fs::create_dir_all(&cache_dir).await?;

        let install_path = if let Ok(current_meshx) = current_meshx {
            let backup_path = cache_dir.join("meshx_backup");
            tokio::fs::copy(&current_meshx, &backup_path).await?;
            debug!(
                backup_path = ?backup_path.display(),
                "backing up current meshx binary"
            );

            // On unix, need to write to a temp file and then atomically replace
            let tmp_path = current_meshx.with_extension("tmp_upgrade");
            {
                let mut f = fs::File::create(&tmp_path).await?;
                f.write_all(&binary_bytes).await?;
                debug!(
                    path = ?tmp_path.display(),
                    "wrote new meshx binary to temporary file",
                );
            }
            #[cfg(unix)]
            {
                ...
                trace!(
                    ?tmp_path,
                    "set permissions for new meshx binary to 755 (rwxr-xr-x)"
                );
            }

            match tokio::fs::copy(&tmp_path, &current_meshx).await {
                Ok(_) => {
                    debug!(
                        ?current_meshx,
                        "successfully replaced current meshx binary with new version"
                    );
                    current_meshx
                }
                Err(e) => {
                    error!(
                        ?e,
                        "failed to replace current meshx binary, referencing temporary file instead"
                    );
                    tmp_path
                }
            }
            ...
            warn!(
                "Cannot find installed meshx binary, assuming installation in a non-standard location."
            );
            ...
            tokio::fs::write(&install_path, &binary_bytes)
                .await
                .context("failed to write new meshx binary")?;
            install_path
        };

        Ok(CommandOutput::ok(
            format!("meshx upgraded to {tag_name}", tag_name = release.tag_name),
            Some(json!(
                {
                    "version": release.tag_name,
                    "backup_path": cache_dir.join("meshx_backup").display().to_string(),
...
impl UpdateCommand {
    /// Get the current version of meshx
...
pub async fn fetch_latest_release_public() -> anyhow::Result<Release> {
    ...
    let client = Client::builder().user_agent("meshx-self-upgrade").build()?;
    let resp = client.get(url).send().await?.error_for_status()?;
    Ok(resp.json().await?)
}
...
impl UpdateConfig {
    ...
    fn create_client(&self) -> anyhow::Result<Client> {
        ...
        headers.insert(USER_AGENT, HeaderValue::from_static("meshx-self-upgrade"));
