...
pub fn path_relative_from(path: impl AsRef<Utf8Path>, base: impl AsRef<Utf8Path>) -> Result<Utf8PathBuf> {
    fn inner(path: &Utf8Path, base: &Utf8Path) -> Result<Utf8PathBuf> {
        ...
        diff_utf8_paths(&path, &base).ok_or_else(|| anyhow!("unable to compute relative path to {path} from {base}"))
    }
...
///
pub fn path_relative_from_file(path: impl AsRef<Utf8Path>, file: impl AsRef<Utf8Path>) -> Result<Utf8PathBuf> {
...
/// ```
///
pub fn resolve_path_from_file(path: impl AsRef<Utf8Path>, resolve_from: impl AsRef<Utf8Path>) -> Result<Utf8PathBuf> {
...
/// ```
///
pub fn resolve_path(path: impl AsRef<Utf8Path>, resolve_from: impl AsRef<Utf8Path>) -> Result<Utf8PathBuf> {
    fn inner(path: &Utf8Path, resolve_from: &Utf8Path) -> Result<Utf8PathBuf> {
        if path.is_absolute() {
            Ok(path.to_owned())
        } else {
...
fn normalize_path_impl<'a>(path_components: impl IntoIterator<Item = Utf8Component<'a>>) -> Result<Utf8PathBuf> {
    let result = path_components
        .into_iter()
            ...
            match component {
                ...
                abs_root @ Utf8Component::RootDir | abs_root @ Utf8Component::Prefix(_) => {
                    if components.is_empty() {
                        ...
                        return Err(anyhow!("Encountered a path root that wasn't in the root position"));
                    }
                }
            }
            Ok(components)
