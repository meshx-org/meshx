...
pub use crate::{
    args::{PackageArchiveCreateCommand, PackageArchiveExtractCommand, PackageBuildCommand},
    package_archive::{
         cmd_package_archive_create, cmd_package_archive_extract,
    },
...
pub(crate) fn to_writer_json_pretty(writer: impl Write, value: impl Serialize) -> serde_json::Result<()> {
...
///
/// `dst`: `srcs_0` `srcs_1` ...
pub(crate) fn write_depfile(path: &Path, dst: &Utf8Path, srcs: impl Iterator<Item = String>) -> Result<()> {
    ...
    let dep_str = format!(
        ...
        srcs.map(|x| convert_to_depfile_filepath(x.as_str()))
            .collect::<Vec<_>>()
            .join(" "),
    );
