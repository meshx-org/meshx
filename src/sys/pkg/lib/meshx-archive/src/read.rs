...
use {
    crate::{
        DirectoryEntry, Error, Index, IndexEntry, DIRECTORY_ENTRY_LEN, DIR_CHUNK_TYPE, DIR_NAMES_CHUNK_TYPE,
    ...
    T: Read + Seek,
{
    ...
    pub fn new(mut source: T) -> Result<Self, Error> {
        ...
        let (dir_index, dir_name_index, end_of_last_non_content_chunk) = Self::read_index_entries(&mut source, &index)?;
        ...
        source
            .seek(SeekFrom::Start(dir_index.offset.get()))
            .map_err(Error::Seek)?;
        source
            .read_exact(directory_entries.as_bytes_mut())
            .map_err(Error::Read)?;
        ...
        source
            .seek(SeekFrom::Start(dir_name_index.offset.get()))
            .map_err(Error::Seek)?;
    ...
    fn read_index_header(source: &mut T) -> Result<Index, Error> {
        ...
        } else if index.length.get() % INDEX_ENTRY_LEN != 0 || INDEX_LEN.checked_add(index.length.get()).is_none() {
    ...
    fn read_index_entries(source: &mut T, index: &Index) -> Result<(IndexEntry, IndexEntry, u64), Error> {
    ...
    pub fn read_file(&mut self, path: &[u8]) -> Result<Vec<u8>, Error> {
        ...
        let _: u64 = self
            .source
            .seek(SeekFrom::Start(entry.data_offset.get()))
            .map_err(Error::Seek)?;
    ...
    pub fn get_size(&mut self, path: &[u8]) -> Result<u64, Error> {
        Ok(
            crate::find_directory_entry(&self.directory_entries, &self.path_data, path)?
                .data_length
                .get(),
        )
    }
...
#[cfg(test)]
mod tests {
    use {super::*, crate::tests::example_archive, assert_matches::assert_matches, std::io::Cursor};
