...
impl quote::ToTokens for Executor {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        tokens.extend(match self {
            Executor::None => quote! { ::meshx::main_not_async(func) },
            Executor::Test => quote! { ::meshx::test_not_async(func) },
            Executor::Singlethreaded => quote! { ::meshx::main_singlethreaded(func) },
            Executor::Multithreaded(n) => quote! { ::meshx::main_multithreaded(func, #n) },
            Executor::SinglethreadedTest => quote! { ::meshx::test_singlethreaded(func) },
            Executor::MultithreadedTest(n) => quote! { ::meshx::test_multithreaded(func, #n) },
            Executor::UntilStalledTest => quote! { ::meshx::test_until_stalled(func) },
        })
    }
}
...
impl quote::ToTokens for Interest {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        /*tokens.extend(match self.min_severity {
            None => quote! { ::meshx::Interest::default() },
            Some(severity) => match severity {
                Severity::Trace => quote! {
                    ::meshx::Interest {
                        min_severity: Some(::meshx::Severity::Trace),
                        ..Default::default()
                    }
                },
                Severity::Debug => quote! {
                    ::meshx::Interest {
                        min_severity: Some(::meshx::Severity::Debug),
                        ..Default::default()
                    }
                },
                Severity::Info => quote! {
                    ::meshx::Interest {
                        min_severity: Some(::meshx::Severity::Info),
                        ..Default::default()
                    }
                },
                Severity::Warn => quote! {
                    ::meshx::Interest {
                        min_severity: Some(::meshx::Severity::Warn),
                        ..Default::default()
                    }
                },
                Severity::Error => quote! {
                    ::meshx::Interest {
                        min_severity: Some(::meshx::Severity::Error),
                        ..Default::default()
                    }
                },
                Severity::Fatal => quote! {
                    ::meshx::Interest {
                        min_severity: Some(::meshx::Severity::Fatal),
                        ..Default::default()
                    }
                },
            },
    ...
    fn finish(self) -> TokenStream {
        ...
        let init_logging = if !self.logging {
            ...
            if self.executor.is_some() {
                quote!(::meshx::init_logging_for_test_with_executor(func, #logging_options))
            } else {
                quote!(::meshx::init_logging_for_test_with_threads(func, #logging_options))
            }
        } else {
            let logging_options = quote! {
                ::meshx::LoggingOptions {
            ...
            if self.executor.is_some() {
                quote!(::meshx::init_logging_for_component_with_executor(func, #logging_options))
            } else {
                quote!(::meshx::init_logging_for_component_with_threads(func, #logging_options))
            }
        };
        ...
        let adapt_main = match (self.executor.is_test(), inputs.len()) {
            ...
            (false, 1) => quote! { ::meshx::adapt_to_parse_arguments(#func) },
            // Test function, no arguments - adapt by taking the run number and discarding it.
            (true, 0) => quote! { ::meshx::adapt_to_take_test_run_number(#func) },
