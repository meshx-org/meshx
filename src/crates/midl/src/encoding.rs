...
//! MIDL encoding and decoding.
// TODO(fxbug.dev/118834): This file is too big. Split it into smaller files.
use {
    crate::endpoints::ProtocolMarker,
    crate::handle::{Handle, HandleBased, HandleDisposition, HandleInfo, HandleOp, ObjectType, Rights, Status},
    crate::{Error, Result},
    bitflags::bitflags,
    fiber_status as fx_status, fiber_types as fx_types,
...
/// `T::Owned` is the MIDL type's "Rust type". For example, for the MIDL type
...
/// to represent MIDL type information such as the element type or constraints.
///
/// # Safety
///
...
pub unsafe trait TypeMarker: 'static + Sized {
    /// The owned Rust type which this MIDL type decodes into.
    ...
    /// Returns true if the memory layout of `Self::Owned` matches the MIDL wire
...
/// A Rust type that can be encoded as the MIDL type `T`.
///
/// # Safety
///
...
/// A Rust type that can be decoded from the MIDL type `T`.
...
/// The maximum number of handles allowed in a MIDL message. Note that this
...
///
/// MIDL conformance tests are used to validate that there are no uninitialized
...
pub enum WireFormatVersion {
    ...
    /// - RFC-0114: Inlining small values in MIDL envelopes
    V2,
}
...
/// MIDL uses `Context` to coordinate soft migrations, so improper uses of it
...
fn default_encode_context() -> Context {
    Context {
        wire_format_version: WireFormatVersion::V2,
    }
}
...
impl<'a> Encoder<'a> {
    /// MIDL-encodes `x` into the provided data and handle buffers.
    #[inline]
    ...
    /// MIDL-encodes `x` into the provided data and handle buffers, using the
    /// specified encoding context.
    ///
    /// WARNING: Do not call this directly unless you know what you're doing.
    /// MIDL uses `Context` to coordinate soft migrations, so improper uses of
...
impl<'a> Decoder<'a> {
    /// Decodes a value of MIDL type `T` into the Rust type `T::Owned` from the
    ...
    pub fn check_inline_envelope_padding(&self, value_offset: usize, value_len: usize) -> Result<()> {
        ...
        let valid_padding = unsafe {
            match value_len {
                ...
                2 => *self.buf.get_unchecked(value_offset + 2) == 0 && *self.buf.get_unchecked(value_offset + 3) == 0,
        ...
        if valid_padding {
            ...
            Err(Error::NonZeroPadding {
                padding_start: value_offset + value_len,
            })
        }
    }
    ...
    #[inline]
    pub fn take_next_handle(&mut self, expected_object_type: ObjectType, expected_rights: Rights) -> Result<Handle> {
        ...
        let handle = self.consume_handle_info(handle_info, expected_object_type, expected_rights)?;
        self.next_handle += 1;
        Ok(handle)
    }
...
/// A fake MIDL type that can encode from and decode into any Rust type. This
...
/// A MIDL type representing an empty payload (0 bytes).
...
/// The MIDL type used for an empty success variant in a result union. Result
...
unsafe impl<'a, T: ResourceTypeMarker, const N: usize> Encode<Array<T, N>> for &'a mut [T::Owned; N] {
    #[inline]
    unsafe fn encode(self, encoder: &mut Encoder<'_>, offset: usize, depth: Depth) -> Result<()> {
        encoder.debug_check_bounds::<Array<T, N>>(offset);
...
// TODO(fxbug.dev/122199): This is only needed for encoding vector<array<T, N>>
// in the old types. Remove once migrated off iterators.
unsafe impl<'a, T: ResourceTypeMarker, const N: usize> Encode<Array<T, N>> for ResourceArray<[T::Owned; N]> {
    #[inline]
    unsafe fn encode(mut self, encoder: &mut Encoder<'_>, offset: usize, depth: Depth) -> Result<()> {
        encoder.debug_check_bounds::<Array<T, N>>(offset);
...
/// The maximum vector bound, corresponding to the `MAX` constraint in MIDL.
pub const MAX_BOUND: usize = usize::MAX;

/// The MIDL type `vector<T>:N`.
pub struct Vector<T: TypeMarker, const N: usize>(PhantomData<T>);

/// The MIDL type `vector<T>` or `vector<T>:MAX`.
...
unsafe impl<'a, T: ResourceTypeMarker, const N: usize> Encode<Vector<T, N>> for &'a mut [T::Owned] {
    #[inline]
    unsafe fn encode(self, encoder: &mut Encoder<'_>, offset: usize, depth: Depth) -> Result<()> {
        encoder.debug_check_bounds::<Vector<T, N>>(offset);
...
unsafe impl<T: TypeMarker, const N: usize, E: Encode<T>, I: ExactSizeIterator<Item = E>> Encode<Vector<T, N>>
    for Iterator<I>
{
    #[inline]
    unsafe fn encode(self, encoder: &mut Encoder<'_>, offset: usize, depth: Depth) -> Result<()> {
...
/// The MIDL type `string:N`.
pub struct BoundedString<const N: usize>;

/// The MIDL type `string` or `string:MAX`.
...
/// The MIDL type `zx.handle:<OBJECT_TYPE, RIGHTS>`, or a `client_end` or `server_end`.
...
/// for the MIDL types `client_end:P` and `server_end:P`.
pub type Endpoint<T> =
    HandleType<T, { crate::ObjectType::CHANNEL.into_raw() }, { crate::Rights::CHANNEL_DEFAULT.bits() }>;
...
impl<T: 'static + HandleBased, const OBJECT_TYPE: u32, const RIGHTS: u32> Decode<HandleType<T, OBJECT_TYPE, RIGHTS>>
    for T
{
    #[inline(always)]
    fn new_empty() -> Self {
...
/// The MIDL type `T:optional` where `T` is a vector, string, handle, or client/server end.
pub struct Optional<T: TypeMarker>(PhantomData<T>);

/// The MIDL type `T:optional` where `T` is a union.
...
unsafe impl<'a, T: TypeMarker, E: Encode<T>> Encode<Boxed<T>> for Option<E> {
    #[inline]
    unsafe fn encode(self, encoder: &mut Encoder<'_>, offset: usize, mut depth: Depth) -> Result<()> {
...
#[inline]
unsafe fn prepare_envelope(inline_size: usize, encoder: &mut Encoder<'_>, offset: usize) -> (usize, FinishEnvelopeFn) {
...
type FinishEnvelopeFn = unsafe fn(encoder: &mut Encoder<'_>, offset: usize, bytes_before: usize, handles_before: usize);
...
#[doc(hidden)] // only exported for use in macros or generated code
#[inline(always)]
pub unsafe fn decode_envelope_header(decoder: &mut Decoder<'_>, offset: usize) -> Result<Option<(bool, u32, u32)>> {
...
/// Decodes a MIDL envelope and skips over any out-of-line bytes and handles.
#[doc(hidden)] // only exported for use in macros or generated code
#[inline]
pub unsafe fn decode_unknown_envelope(decoder: &mut Decoder<'_>, offset: usize, mut depth: Depth) -> Result<()> {
...
/// MIDL bits. The struct must have been generated by the bitflags crate.
#[macro_export]
macro_rules! midl_bits {
    (
        name: $name:ident,
        prim_ty: $prim_ty:ty,
...
/// Implements encoding and decoding traits for a Rust enum representing a MIDL enum.
#[macro_export]
macro_rules! midl_enum {
    (
        name: $name:ident,
        prim_ty: $prim_ty:ty,
        // Must provide `strict: true` or `flexible: true`.
...
/// Implements encoding and decoding traits for a Rust struct representing a MIDL struct.
#[macro_export]
macro_rules! midl_struct {
    (empty: true, $($tok:tt)*) => {
        $crate::midl_struct_impl_empty!($($tok)*);
    };
    (copy: true, $($tok:tt)*) => {
        $crate::midl_struct_impl_copy!($($tok)*);
        $crate::midl_struct_impl_tuple!($($tok)*);
    };
    ($($tok:tt)*) => {
        $crate::midl_struct_impl_noncopy!($($tok)*);
        $crate::midl_struct_impl_tuple!($($tok)*);
    };
}
...
#[doc(hidden)] // only exported for use in macros or generated code
#[macro_export]
macro_rules! midl_struct_impl_empty {
    (
        name: $name:ident,
...
#[doc(hidden)] // only exported for use in macros or generated code
#[macro_export]
macro_rules! midl_struct_impl_copy {
    (
        name: $name:ident,
        $(resource: $resource:tt,)?
        members: [$(
        ...
        unsafe impl $crate::encoding::Encode<$name> for $crate::switch! {
            ...
            unsafe fn encode(self, encoder: &mut $crate::encoding::Encoder<'_>, offset: usize, _depth: $crate::encoding::Depth) -> $crate::Result<()> {
                ...
                unsafe {
                    ...
                    // Zero out padding regions. Unlike `midl_struct_impl_noncopy!`, this must be
...
#[doc(hidden)] // only exported for use in macros or generated code
#[macro_export]
macro_rules! midl_struct_impl_noncopy {
    (
        name: $name:ident,
        $(resource: $resource:tt,)?
        members: [$(
...
#[doc(hidden)] // only exported for use in macros or generated code
#[macro_export]
macro_rules! midl_struct_impl_tuple {
    (
        name: $name:ident,
        $(resource: $resource:tt,)?
        members: [$(
            ...
            $crate::encoding::Encode<$name> for ($($member_typevar,)*)
        {
            ...
            unsafe fn encode(self, encoder: &mut $crate::encoding::Encoder<'_>, offset: usize, depth: $crate::encoding::Depth) -> $crate::Result<()> {
                ...
                // TODO(fxbug.dev/123341): Make this consistent with midl_struct_impl_copy.
                match encoder.context().wire_format_version {
                    $crate::encoding::WireFormatVersion::V1 => {
                        $(
                            unsafe {
...
/// Implements encoding and decoding traits for a Rust struct representing a
/// MIDL table. All the struct's fields must be `Option`s, except for the
/// `pub __non_exhaustive: ()` field.
#[macro_export]
macro_rules! midl_table {
    (
        name: $name:ident,
        $(resource: $resource:tt,)?
        members: [$(
...
/// Implements encoding and decoding traits for a Rust enum representing a MIDL union.
#[macro_export]
macro_rules! midl_union {
    (
        name: $name:ident,
        $(resource: $resource:tt,)?
        members: [$(
        ...
        unsafe impl $crate::encoding::TypeMarker for $name {
            type Owned = Self;
            $crate::midl_union_impl_type_fns!();
        }
        $crate::impl_value_or_resource_type!($name, $($resource)?);
        unsafe impl $crate::encoding::Encode<$name> for $crate::switch! {
            $($resource)? => { &mut $name }
        ...
        impl $crate::encoding::Decode<$name> for $name {
            $crate::midl_union_impl_decode_fns! {
                ty: $name,
                members: [$(
                    {
...
/// Helper macro that implements `TypeMarker` functions for a MIDL union.
#[doc(hidden)] // only exported for use in macros or generated code
#[macro_export]
macro_rules! midl_union_impl_type_fns {
    () => {
        #[inline(always)]
        fn inline_align(_context: $crate::encoding::Context) -> usize {
            8
...
// Helper macro that implements `Decode` functions for a MIDL union.
#[doc(hidden)] // only exported for use in macros or generated code
#[macro_export]
macro_rules! midl_union_impl_decode_fns {
    (
        ty: $ty:ty,
        members: [$(
            {
...
#[doc(hidden)] // only exported for use in macros or generated code
#[inline]
pub unsafe fn decode_union_inline_portion(decoder: &mut Decoder, offset: usize) -> Result<(u64, bool, u32, u32)> {
...
/// The MIDL union generated for strict two-way methods with errors.
pub struct ResultType<T: TypeMarker, E: TypeMarker>(PhantomData<(T, E)>);

/// The MIDL union generated for flexible two-way methods without errors.
pub struct FlexibleType<T: TypeMarker>(PhantomData<T>);

/// The MIDL union generated for flexible two-way methods with errors.
...
/// Internal MIDL framework error type used to identify unknown methods.
#[derive(Copy, Clone, Debug, Eq, PartialEq, Ord, PartialOrd, Hash)]
#[repr(i32)]
pub enum FrameworkErr {
    /// Method was not recognized.
    UnknownMethod = fx_types::FX_ERR_NOT_SUPPORTED,
}
...
impl FrameworkErr {
    ...
    fn from_primitive(prim: i32) -> Option<Self> {
        match prim {
            fx_types::FX_ERR_NOT_SUPPORTED => Some(Self::UnknownMethod),
            _ => None,
        }
    }
    #[inline(always)]
...
impl<T> Flexible<T> {
    ...
    /// Converts to a `midl::Result`, mapping framework errors to `midl::Error`.
    pub fn into_result<P: ProtocolMarker>(self, method_name: &'static str) -> Result<T> {
        match self {
            Flexible::Ok(ok) => Ok(ok),
            Flexible::FrameworkErr(FrameworkErr::UnknownMethod) => Err(Error::UnsupportedMethod {
                method_name,
...
impl<T, E> FlexibleResult<T, E> {
    ...
    /// Converts to a `midl::Result`, mapping framework errors to `midl::Error`.
    pub fn into_result<P: ProtocolMarker>(self, method_name: &'static str) -> Result<std::result::Result<T, E>> {
        match self {
            FlexibleResult::Ok(ok) => Ok(Ok(ok)),
            FlexibleResult::DomainErr(err) => Ok(Err(err)),
            FlexibleResult::FrameworkErr(FrameworkErr::UnknownMethod) => Err(Error::UnsupportedMethod {
                method_name,
                protocol_name: P::DEBUG_NAME,
            }),
        }
...
/// The body of a MIDL Epitaph
#[derive(Copy, Clone, Debug, Eq, PartialEq)]
pub struct EpitaphBody {
    /// The error status.
    pub error: fx_status::Status,
}
...
unsafe impl TypeMarker for fx_status::Status {
    type Owned = Self;
    #[inline(always)]
    fn inline_align(_context: Context) -> usize {
        mem::align_of::<fx_status::fx_status_t>()
    }
    #[inline(always)]
    fn inline_size(_context: Context) -> usize {
        mem::size_of::<fx_status::fx_status_t>()
    }
}

impl_value_type_by_copy!(fx_status::Status);
unsafe impl Encode<fx_status::Status> for fx_status::Status {
    #[inline]
    unsafe fn encode(self, encoder: &mut Encoder<'_>, offset: usize, _depth: Depth) -> Result<()> {
        encoder.debug_check_bounds::<fx_status::Status>(offset);
        encoder.write_num::<i32>(self.into_raw(), offset);
        Ok(())
    }
}

impl Decode<fx_status::Status> for fx_status::Status {
    #[inline(always)]
    fn new_empty() -> Self {
        Self::from_raw(0)
    }
    #[inline]
    unsafe fn decode(&mut self, decoder: &mut Decoder<'_>, offset: usize, _depth: Depth) -> Result<()> {
        decoder.debug_check_bounds::<fx_status::Status>(offset);
...
/// The MIDL type for a message consisting of a header `H` and body `T`.
    ...
    for GenericMessage<H::Owned, E>
{
    ...
    unsafe fn encode(self, encoder: &mut Encoder<'_>, offset: usize, depth: Depth) -> Result<()> {
        ...
        self.body
            .encode(encoder, offset + H::inline_size(encoder.context), depth)
    }
}
...
/// The MIDL type for a transaction message with body `T`.
...
/// Header for transactional MIDL messages
#[derive(Copy, Clone, Debug, Eq, PartialEq)]
#[repr(C)]
pub struct TransactionHeader {
    ...
    magic_number: u8,
    /// Ordinal which identifies the MIDL method
    ordinal: u64,
}
...
bitflags! {
    ...
    pub struct AtRestFlags: u16 {
        ...
        /// - Efficient envelopes
        /// - Inlining small values in MIDL envelopes
...
impl TransactionHeader {
    ...
    pub fn decoding_context(&self) -> Context {
        if self.at_rest_flags().contains(AtRestFlags::USE_V2_WIRE_FORMAT) {
            ...
            Context {
                wire_format_version: WireFormatVersion::V1,
            }
        }
    }
}
...
/// Marker trait implemented for MIDL non-resource structs, tables, and unions.
...
/// Marker trait implemented for MIDL resource structs, tables, and unions.
...
/// Header for RFC-0120 persistent MIDL messages.
#[derive(Copy, Clone, Debug, Eq, PartialEq)]
#[repr(C)]
...
// Encodes a MIDL object to bytes following RFC-0120. This only works on
...
#[doc(hidden)]
pub fn persist_with_context<'a, T: ValueTypeMarker>(body: T::Borrowed<'a>, context: Context) -> Result<Vec<u8>> {
...
/// Decodes a MIDL object from bytes following RFC-0120. Must be a non-resource
...
pub fn unpersist<T: Persistable>(bytes: &[u8]) -> Result<T> {
    ...
    //
    // To soft-transition component manager's use of persistent MIDL, we
...
/// Encodes a MIDL object to bytes and wire metadata following RFC-0120. Must be
...
/// Encodes a MIDL object to bytes, handles, and wire metadata following
...
/// Decodes a MIDL object from bytes and wire metadata following RFC-0120. Must
...
/// Decodes a MIDL object from bytes, handles, and wire metadata following
/// RFC-0120. Must be a resource struct, table, or union.
...
pub fn convert_handle_dispositions_to_infos(handle_dispositions: Vec<HandleDisposition>) -> Result<Vec<HandleInfo>> {
...
/// Special MIDL message body for large messages.
#[derive(Copy, Clone, Debug, Eq, PartialEq)]
#[repr(C)]
pub struct LargeMessageInfo {
    ...
    reserved: u32,
    /// The size of the encoded MIDL message in the VMO. Must be a multiple of MIDL alignment.
    msg_byte_count: u64,
}
...
pub fn maybe_overflowing_after_encode(
    ...
    if _write_bytes.len() <= fiber_rust::sys::FX_CHANNEL_MAX_MSG_BYTES as usize {
        return Ok(());
    }
    ...
    let vmo = fiber_rust::Vmo::create(body_size).map_err(|status| Error::LargeMessageCouldNotWriteVmo { status })?;
...
/// Decodes a MIDL value from the given buffers, assuming they came from a
...
pub fn maybe_overflowing_decode<T: TypeMarker>(
    ...
    if vmo_handle_info.object_type != ObjectType::VMO || vmo_handle_info.rights != LARGE_MESSAGE_VMO_RIGHTS {
    ...
    const MAX_MSG_BYTES: usize = fiber_rust::sys::FX_CHANNEL_MAX_MSG_BYTES as usize;
    ...
    Decoder::decode_with_context::<LargeMessageInfo>(ctx, &body_bytes, &mut [], &mut large_message_info)?;
    ...
    let vmo = fiber_rust::Vmo::from(vmo_handle_info.handle);
...
#[inline]
pub fn with_tls_encode_buf<R>(f: impl FnOnce(&mut Vec<u8>, &mut Vec<HandleDisposition<'static>>) -> R) -> R {
    TLS_BUF.with(|buf| {
        let (mut bytes, mut handles) = RefMut::map_split(buf.borrow_mut(), |b| (&mut b.bytes, &mut b.encode_handles));
        if bytes.capacity() == 0 {
...
pub fn with_tls_decode_buf<R>(f: impl FnOnce(&mut Vec<u8>, &mut Vec<HandleInfo>) -> R) -> R {
    TLS_BUF.with(|buf| {
        let (mut bytes, mut handles) = RefMut::map_split(buf.borrow_mut(), |b| (&mut b.bytes, &mut b.decode_handles));
        if bytes.capacity() == 0 {
            bytes.reserve(MIN_TLS_BUF_BYTES_SIZE);
        }
        let res = f(&mut bytes, &mut handles);
...
mod test {
    ...
    // `midl_bits!`, `midl_union!`, etc. To the compiler, it's as if we defined
    ...
    pub fn encode_decode<T: TypeMarker>(ctx: Context, start: impl Encode<T>) -> T::Owned {
        ...
        Decoder::decode_with_context::<T>(ctx, &buf, &mut to_infos(handle_buf), &mut out).expect("Decoding failed");
        out
    }

    #[track_caller]
    fn encode_assert_bytes<T: TypeMarker>(ctx: Context, data: impl Encode<T>, encoded_bytes: &[u8]) {
        let buf = &mut Vec::new();
        let handle_buf = &mut Vec::new();
    ...
    fn result_and_union_compat() {
        ...
        for ctx in CONTEXTS {
            ...
            let handle_buf = &mut Vec::new();
            // result to union
            Encoder::encode_with_context::<Res>(ctx, buf, handle_buf, Ok::<(), i32>(())).expect("Encoding failed");
            let mut out = new_empty!(OkayOrError);
            Decoder::decode_with_context::<OkayOrError>(ctx, buf, &mut to_infos(handle_buf), &mut out)
                .expect("Decoding failed");
            assert_eq!(out, OkayOrError::Okay(Empty));
            Encoder::encode_with_context::<Res>(ctx, buf, handle_buf, Err::<(), i32>(5)).expect("Encoding failed");
            Decoder::decode_with_context::<OkayOrError>(ctx, buf, &mut to_infos(handle_buf), &mut out)
                .expect("Decoding failed");
            assert_eq!(out, OkayOrError::Error(5));
            // union to result
            let mut out: std::result::Result<(), i32> = new_empty!(Res);
            Encoder::encode_with_context::<OkayOrError>(ctx, buf, handle_buf, &OkayOrError::Okay(Empty))
                .expect("Encoding failed");
            Decoder::decode_with_context::<Res>(ctx, buf, &mut to_infos(handle_buf), &mut out)
                .expect("Decoding failed");
            assert_eq!(out, Ok(()));
            Encoder::encode_with_context::<OkayOrError>(ctx, buf, handle_buf, &OkayOrError::Error(3i32))
                .expect("Encoding failed");
            Decoder::decode_with_context::<Res>(ctx, buf, &mut to_infos(handle_buf), &mut out)
                .expect("Decoding failed");
            assert_eq!(out, Err(3));
    ...
    fn decode_result_unknown_tag() {
        ...
        let bytes: &[u8] = &[
            ...
            0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // inline value -----|  NHandles |  Flags ---|
    ...
    fn decode_result_success_invalid_empty_struct() {
        ...
        let bytes: &[u8] = &[
            // Ordinal 1 (success) ----------------------|
            0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // inline value -----|  NHandles |  Flags ---|
    ...
    fn decode_struct_with_invalid_padding_fails() {
        for ctx in CONTEXTS {
            ...
            let buf = &mut Vec::new();
            let handle_buf = &mut Vec::new();
            Encoder::encode_with_context::<Foo>(ctx, buf, handle_buf, foo).expect("Encoding failed");
            buf[1] = 42;
            let out = &mut new_empty!(Foo);
            let result = Decoder::decode_with_context::<Foo>(ctx, buf, &mut to_infos(handle_buf), out);
    ...
    fn direct_copy_struct_encode() {
        ...
        let obj = DirectCopyStruct {
            a: 0x0807060504030201,
            b: 0x0c0b0a09,
            c: 0x0e0d,
            d: 0x100f,
        };
    ...
    fn direct_copy_struct_decode() {
        ...
        for ctx in CONTEXTS {
            ...
            Decoder::decode_with_context::<DirectCopyStruct>(ctx, bytes, &mut [], &mut out).expect("Decoding failed");
            assert_eq!(out, obj);
        }
    }
    ...
    fn encode_decode_transaction_msg() {
        for ctx in CONTEXTS {
            ...
            let buf = &mut Vec::new();
            let handle_buf = &mut Vec::new();
            Encoder::encode_with_context::<TransactionMessageType<Body>>(ctx, buf, handle_buf, start)
                .expect("Encoding failed");
            let (out_header, out_buf) = decode_transaction_header(&buf).expect("Decoding header failed");
            assert_eq!(header, out_header);
            let mut body_out = String::new();
            Decoder::decode_into::<Body>(&header, out_buf, &mut to_infos(handle_buf), &mut body_out)
    ...
    fn direct_decode_transaction_header_strict() {
        ...
        for ctx in CONTEXTS {
            ...
            Decoder::decode_with_context::<TransactionHeader>(ctx, bytes, &mut [], &mut out).expect("Decoding failed");
    ...
    fn direct_encode_large_message_info() {
        ...
        let large_message_info = LargeMessageInfo {
            flags: 0,
            reserved: 0,
            msg_byte_count: 65544,
        };
        for ctx in CONTEXTS {
    ...
    fn direct_decode_large_message_info() {
        ...
        for ctx in CONTEXTS {
            ...
            Decoder::decode_with_context::<LargeMessageInfo>(ctx, bytes, &mut [], &mut out).expect("Decoding failed");
    ...
    fn encode_handle() {
        ...
        for ctx in CONTEXTS {
            let handle = crate::handle::Vmo::create(10).unwrap().into_handle();
            let raw_handle = handle.raw_handle();
            let buf = &mut Vec::new();
            let handle_buf = &mut Vec::new();
            
            Encoder::encode_with_context::<T>(ctx, buf, handle_buf, handle).expect("Encoding failed");
    ...
    fn decode_too_few_handles_unknown_envelope() {
        ...
        let bytes: &[u8] = &[
            // Table Size 1 -----------------------------|
            0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // Table data ptr present -------------------|
    ...
    fn encode_epitaph() {
        for ctx in CONTEXTS {
            ...
            Encoder::encode_with_context::<EpitaphBody>(
                ...
                handle_buf,
                &EpitaphBody {
                    error: fx_status::Status::UNAVAILABLE,
                },
            )
            .expect("encoding failed");
            assert_eq!(buf, &[0xe4, 0xff, 0xff, 0xff, 0x00, 0x00, 0x00, 0x00]);
            let mut out = new_empty!(EpitaphBody);
            Decoder::decode_with_context::<EpitaphBody>(ctx, buf, &mut to_infos(handle_buf), &mut out)
                .expect("Decoding failed");
            assert_eq!(
                EpitaphBody {
                    error: fx_status::Status::UNAVAILABLE
