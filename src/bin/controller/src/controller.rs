...
use controlplane_client::api::{Patch, PatchParams};
use controlplane_client::{Api, Resource, Client, ResourceExt};
use controlplane_core::ListParams;
use controlplane_derive::CustomResource;
use controlplane_runtime::watcher;
use controlplane_runtime::finalizer::{finalizer, Event as Finalizer};
use controlplane_runtime::controller::{Action, Config, Controller};
use controlplane_runtime::events::{Event, EventType, Recorder, Reporter};
...
#[kube(kind = "Document", group = "meshx.co", version = "v1", namespaced)]
...
async fn reconcile(doc: Arc<Document>, ctx: Arc<Context>) -> Result<Action, Error> {
