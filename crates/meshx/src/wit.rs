//! WIT dependency management for meshx components
//!
//! This module provides functionality to fetch and manage WebAssembly Interface Type (WIT)
//! dependencies for meshx components. It integrates with the wasm-pkg-client for
...
/// The default name of the lock file for meshx projects
...
pub struct CommonPackageArgs {
    ...
    /// The path to the cache directory. Defaults to the meshx cache directory.
...
impl CommonPackageArgs {
    ...
    pub async fn load_config(&self) -> anyhow::Result<wasm_pkg_client::Config> {
        ...
        // 2. Path provided by the user via `MESHX` prefixed environment variable
        // 3. Path provided by the users via `WKG` prefixed environment variable
        // 4. Default path to config file in meshx dir
        // 5. Default path to config file from wkg
        match (self.config.as_ref(), std::env::var_os("WKG_CONFIG_FILE")) {
            // We have a config file provided by the user flag or MESHX env var
            (Some(path), _) => {
    ...
    pub async fn load_cache(&self) -> anyhow::Result<FileCache> {
        ...
        // 2. Path provided by the user via `MESHX` prefixed environment variable
        // 3. Path provided by the users via `WKG` prefixed environment variable
        // 4. Default path to cache in meshx dir
        let dir = match (self.cache.as_ref(), std::env::var_os("WKG_CACHE_DIR")) {
            // We have a cache dir provided by the user flag or MESHX env var
...
///
/// The lock file is expected to be at `wkg.lock` in the project root, or `.meshx/meshx.lock` relative to the project directory.
#[instrument(skip_all)]
...
mod tests {
    ...
    fn test_parse_wit_package_name() {
        let (ns, pkgs, ver) = parse_wit_package_name("meshx:test@1.0.0").unwrap();
        assert_eq!(ns, "meshx");
        assert_eq!(pkgs, vec!["test"]);
        ...
        let (ns, pkgs, ver) = parse_wit_package_name("meshx@2.0.0").unwrap();
        assert_eq!(ns, "meshx");
    ...
    fn test_set_override_for_target() {
        ...
        set_override_for_target(&mut overrides, "meshx", &[], path.clone());
        assert!(overrides.contains_key("meshx"));
        assert_eq!(overrides["meshx"].path, Some(path.clone()));
    ...
    fn test_wit_config_deserialization() {
        ...
        assert_eq!(config.sources["meshx:test"], "../shared-wit");
        assert_eq!(
