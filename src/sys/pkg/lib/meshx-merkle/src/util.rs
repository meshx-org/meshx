...
pub fn hash_block(block: &[u8], offset: usize) -> Hash {
    ...
    let bytes: &[u8; 32] = bytes[0..32].try_into().unwrap();
    Hash::from(*bytes)
}
...
///     .parse()
///     .unwrap();
/// let hash_of_single_hash = meshx_merkle::hash_hashes(&vec![data_hash], 0, 0);
/// let hash_of_single_hash_and_zero_hash =
///     meshx_merkle::hash_hashes(&vec![data_hash, zero_hash], 0, 0);
