...
pub struct ListParams {
    ...
    #[serde(default)]
    #[serde(deserialize_with = "from_go_duration")]
...
fn from_go_duration<'de, D>(deserializer: D) -> Result<Option<Duration>, D::Error>
where
    D: Deserializer<'de>,
{
    match Deserialize::deserialize(deserializer)? {
        None => Ok(None),
        Some(duration) => Ok(Some(Duration::from_nanos(parse_duration(duration).unwrap() as u64))),
    }
}
...
impl ListParams {
    ...
    /// Defaults to 290s
    #[must_use]
    pub fn timeout(mut self, timeout_secs: Duration) -> Self {
...
/// Usage:
/// ```
/// use controlplane_api::params::GetParams;
...
impl WatchParams {
    pub(crate) fn validate(&self) -> Result<(), Error> {
        ...
        if let (Some(_), None) = (self.send_initial_events, self.bookmarks) {
            return Err(Error::Validation(
    ...
    pub(crate) fn populate_qp(&self, qp: &mut form_urlencoded::Serializer<String>) {
        ...
        if let Some(_) = self.bookmarks {
            qp.append_pair("allowWatchBookmarks", "true");
        }
        if let Some(_) = self.send_initial_events {
...
impl WatchParams {
    ...
    pub fn initial_events(mut self) -> Self {
        self.send_initial_events = Some(true);
