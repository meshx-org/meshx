...
use {
    crate::{
        epitaph::ChannelEpitaphExt, AsHandleRef, AsyncChannel, Channel, Error, Handle, HandleBased, HandleRef,
        OnSignals, ServeInner,
    },
    fiber_status as fx_status,
    futures::{self, Future, FutureExt, Stream, TryFutureExt, TryStream, TryStreamExt},
    meshx_async as fasync,
    std::convert::From,
...
/// need to add the `@discoverable` attribute to the MIDL protocol, like this:
///
/// ```midl
/// @discoverable
/// protocol MyProtocol { ... };
...
impl<T> ServerEnd<T> {
    ...
    pub fn close_with_epitaph(self, status: fx_status::Status) -> Result<(), Error> {
