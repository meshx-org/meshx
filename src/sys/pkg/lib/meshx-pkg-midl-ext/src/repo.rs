...
use {
    ...
    http::Uri,
    super::http_uri_ext::HttpUriExt as _,
...
impl<'de> serde::Deserialize<'de> for MirrorConfig {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        let blob_mirror_url = if let Some(blob_mirror_url) = blob_mirror_url {
            blob_mirror_url
                .parse::<Uri>()
                .map_err(|e| serde::de::Error::custom(format!("bad uri string: {:?}: {}", blob_mirror_url, e)))?
        } else {
...
impl MirrorConfigBuilder {
    ...
    pub fn mirror_url(mut self, mirror_url: impl Into<http::Uri>) -> Result<Self, (Self, MirrorConfigError)> {
    ...
    pub fn blob_mirror_url(mut self, blob_mirror_url: impl Into<http::Uri>) -> Result<Self, (Self, MirrorConfigError)> {
...
/*impl TryFrom<fidl::MirrorConfig> for MirrorConfig {
    ...
    fn try_from(other: fidl::MirrorConfig) -> Result<Self, RepositoryParseError> {
        let mirror_url = other
            .mirror_url
            .ok_or(RepositoryParseError::MirrorUrlMissing)?
            .parse::<Uri>()?;
        if mirror_url.scheme().is_none() {
...
/*impl From<MirrorConfig> for fidl::MirrorConfig {
    fn from(config: MirrorConfig) -> Self {
        let blob_mirror_url =
            normalize_blob_mirror_url(&config.mirror_url, &config.blob_mirror_url).map(|url| url.to_string());
        Self {
...
fn normalize_blob_mirror_url<'a>(mirror_url: &http::Uri, blob_mirror_url: &'a http::Uri) -> Option<&'a http::Uri> {
...
pub struct RepositoryConfig {
    repo_url: meshx_url::RepositoryUrl,
...
impl RepositoryConfig {
    pub fn repo_url(&self) -> &meshx_url::RepositoryUrl {
        &self.repo_url
    }
...
/*impl TryFrom<fidl::RepositoryConfig> for RepositoryConfig {
    ...
    fn try_from(other: fidl::RepositoryConfig) -> Result<Self, RepositoryParseError> {
        let repo_url: meshx_url::RepositoryUrl = other
            .repo_url
        ...
        let storage_type = other
            .storage_type
            .map(|r| r.into())
            .unwrap_or(RepositoryStorageType::Ephemeral);
...
impl RepositoryConfigBuilder {
    pub fn new(repo_url: meshx_url::RepositoryUrl) -> Self {
    ...
    pub fn repo_url(mut self, repo_url: meshx_url::RepositoryUrl) -> Self {
...
pub struct RepositoryUrl {
    url: meshx_url::RepositoryUrl,
}

impl RepositoryUrl {
    pub fn url(&self) -> &meshx_url::RepositoryUrl {
        &self.url
    }
}

impl From<meshx_url::RepositoryUrl> for RepositoryUrl {
    fn from(url: meshx_url::RepositoryUrl) -> Self {
        Self { url }
    }
}
...
/*impl TryFrom<&fidl::RepositoryUrl> for RepositoryUrl {
    ...
    fn try_from(other: &fidl::RepositoryUrl) -> Result<Self, RepositoryUrlParseError> {
        Ok(Self {
            url: other.url.parse().map_err(RepositoryUrlParseError::InvalidRepoUrl)?,
        })
    }
}
...
mod tests {
    ...
    fn test_mirror_config_from_fidl_rejects_urls_without_schemes() {
        ...
        assert_matches!(
            MirrorConfig::try_from(as_fidl),
            Err(RepositoryParseError::MirrorConfig(
                MirrorConfigError::MirrorUrlMissingScheme
            ))
        );
        ...
        assert_matches!(
            MirrorConfig::try_from(as_fidl),
            Err(RepositoryParseError::MirrorConfig(
                MirrorConfigError::BlobMirrorUrlMissingScheme
            ))
        );
    }
    ...
    fn test_mirror_config_builder() {
        let builder = MirrorConfigBuilder::new("http://example.com/".parse::<Uri>().unwrap()).unwrap();
        assert_eq!(
            builder.clone().build(),
            MirrorConfig {
    ...
    fn test_repository_config_builder() {
        let repo_url = meshx_url::RepositoryUrl::parse("fuchsia-pkg://fuchsia.com").unwrap();
    ...
    fn test_repository_url_into_fidl() {
        let url = RepositoryUrl {
            url: "fuchsia-pkg://fuchsia.com".parse().unwrap(),
        };
        let as_fidl: fidl::RepositoryUrl = url.into();
        assert_eq!(
            as_fidl,
            fidl::RepositoryUrl {
                url: "fuchsia-pkg://fuchsia.com".to_owned()
            }
        );
    }
    ...
    fn test_repository_url_from_fidl() {
        let as_fidl = fidl::RepositoryUrl {
            url: "fuchsia-pkg://fuchsia.com".to_owned(),
        };
        assert_matches!(
    ...
    fn test_repository_url_from_fidl_with_bad_url() {
        let as_fidl = fidl::RepositoryUrl {
            url: "invalid-scheme://fuchsia.com".to_owned(),
        };
        assert_matches!(
            RepositoryUrl::try_from(&as_fidl),
            Err(RepositoryUrlParseError::InvalidRepoUrl(
                meshx_url::ParseError::InvalidScheme
            ))
        );
    }
