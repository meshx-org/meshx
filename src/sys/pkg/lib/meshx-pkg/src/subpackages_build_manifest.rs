    ...
    pub fn to_subpackages(&self) -> Result<Vec<(RelativePackageUrl, Hash, Utf8PathBuf)>> {
        ...
        for entry in &self.0.entries {
            ...
            let url = match &entry.kind {
                ...
                SubpackagesBuildManifestEntryKind::Empty | SubpackagesBuildManifestEntryKind::MetaPackageFile(_) => {
...
impl Serialize for SubpackagesBuildManifestEntry {
        ...
        S: serde::Serializer,
    {
        ...
        match &self.kind {
            ...
            SubpackagesBuildManifestEntryKind::MetaPackageFile(path) => helper.meta_package_file = Some(path),
        }
...
impl SubpackagesBuildManifestEntry {
    ...
    pub fn new(kind: SubpackagesBuildManifestEntryKind, package_manifest_file: Utf8PathBuf) -> Self {
        Self {
            kind,
            package_manifest_file,
        }
...
mod tests {
    ...
    fn test_deserialize() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                SubpackagesBuildManifestEntry {
                    kind: SubpackagesBuildManifestEntryKind::MetaPackageFile(pkg2_meta_package_file),
    ...
    fn test_serialize_deserialize() {
        ...
        let mut bytes = vec![];
        let () = manifest.serialize(&mut bytes).unwrap();
        let deserialized = SubpackagesBuildManifest::deserialize(io::BufReader::new(bytes.as_slice())).unwrap();
