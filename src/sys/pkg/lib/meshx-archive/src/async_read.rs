...
use {
    ...
    meshx_fs::file::{AsyncGetSize, AsyncGetSizeExt, AsyncReadAt, AsyncReadAtExt},
    ...
    T: AsyncReadAt + AsyncGetSize + Unpin,
{
    ...
    async fn read_index_header(source: &mut T) -> Result<Index, Error> {
        ...
        source
            .read_at_exact(0, index.as_bytes_mut())
            .await
            .map_err(Error::Read)?;
    ...
    async fn read_index_entries(source: &mut T, index: &Index) -> Result<(IndexEntry, IndexEntry, u64), Error> {
        ...
        for i in 0..index.length.get() / INDEX_ENTRY_LEN {
            ...
            source
                .read_at_exact(entry_offset, entry.as_bytes_mut())
                .await
                .map_err(Error::Read)?;
...
mod tests {
    use {
        super::*, crate::tests::example_archive, assert_matches::assert_matches, futures::io::Cursor,
