...
export abstract class Binding<T> {
    ...
    private handleReadable(): void {
        ...
        if (result.bytes?.byteLength == 0) {
            throw new MidlError(`Unexpected empty message or error: ${result} from channel ${this._reader.channel}`)
        }
    ...
    sendMessage(response: OutgoingMessage): void {
        ...
        this._reader.channel.writeEtc(response.buffer, response.handleDispositions)
    }
}
...
export class ProxyController<T> {
    ...
    public sendMessage(message: OutgoingMessage): void {
        ...
        const status = this._reader.channel!.writeEtc(message.buffer, message.handleDispositions)
        if (status != Status.OK) {
