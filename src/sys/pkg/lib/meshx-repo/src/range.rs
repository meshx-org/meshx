...
impl Range {
    ...
    pub fn to_http_request_header(&self) -> Option<HeaderValue> {
        ...
        let header = HeaderValue::from_str(&value).expect("header to only contain ASCII characters");
...
impl ContentRange {
    ...
    pub fn total_len(&self) -> u64 {
        match self {
            ContentRange::Full { complete_len } | ContentRange::Inclusive { complete_len, .. } => *complete_len,
        }
    }
    ...
    pub fn to_http_content_range_header(&self) -> Option<HeaderValue> {
        match self {
            ...
            ContentRange::Inclusive {
                ...
                // The unwrap should be safe here since HeaderValue only fails if there are
                // non-ascii characters in the string.
                let header = HeaderValue::from_str(&value).expect("header to not contain illegal characters");
...
mod tests {
    ...
    fn test_range_to_http_range_header() {
        for (range, expected) in [
            (Range::Full, None),
            (
                Range::From { first_byte_pos: 5 },
                Some(HeaderValue::from_static("bytes=5-")),
            ),
            (
                Range::Inclusive {
                    first_byte_pos: 5,
    ...
    fn test_content_range_from_http_content_range_fails_correctly() {
        for header in ["", "bytes -/10", "not-bytes 1-5/10", "bytes 0x1-0x2/0x3"] {
            let header = HeaderValue::from_static(header);
            assert_matches!(ContentRange::from_http_content_range_header(&header), Err(Error::Parse));
        }

        let header =
            HeaderValue::from_static("bytes 184467440737095516150-184467440737095516151/184467440737095516152");
        assert_matches!(
            ContentRange::from_http_content_range_header(&header),
            Err(Error::Overflow)
        );
    ...
    fn test_content_length_contains_range_inclusive() {
        assert!(ContentLength::new(1).contains_range(Range::Inclusive {
            first_byte_pos: 0,
            last_byte_pos: 0
        }));
        assert!(ContentLength::new(100).contains_range(Range::Inclusive {
            first_byte_pos: 0,
            last_byte_pos: 99
        }));
        assert!(ContentLength::new(100).contains_range(Range::Inclusive {
        ...
        assert!(!ContentLength::new(0).contains_range(Range::Inclusive {
            first_byte_pos: 0,
            last_byte_pos: 0
        }));
        assert!(!ContentLength::new(100).contains_range(Range::Inclusive {
