...
use super::instrumentation::Collector;
use super::packets::PacketReceiverMap;
use super::time::Time;
...
impl Inner {
    ...
    pub fn set_local(self: Arc<Self>, timers: TimerHeap) {
        EXECUTOR.with(|e| {
            ...
            assert!(e.is_none(), "Cannot create multiple MeshX Executors");
            *e = Some((self, timers));
        });
    }

    pub fn poll_ready_tasks(&self) {
    ...
    pub fn notify_id(&self, id: u64) {
        let up = fx::UserPacket::from_u8_array([0; 32]);
        let packet = fx::Packet::from_user_packet(id, 0 /* status??? */, up);
    ...
    pub fn now(&self) -> Time {
        match &self.time {
            ExecutorTime::RealTime => Time::from_fx(fx::Time::get_monotonic()),
...
impl EHandle {
    ...
    pub fn local() -> Self {
        let inner = EXECUTOR
            ...
            .expect("MeshX Executor must be created first");
    ...
    /// Get a reference to the MeshX `fx::Port` being used to listen for events.
