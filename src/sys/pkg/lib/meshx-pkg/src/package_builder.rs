...
use {
    crate::{
        path_to_string::PathToStringExt, MetaContents, MetaPackage, MetaSubpackages, PackageBuildManifest,
        PackageManifest, RelativeTo, SubpackageEntry,
    },
...
impl PackageBuilder {
        ...
        for blob in original_manifest.blobs() {
            if blob.path == PackageManifest::META_FAR_BLOB_PATH {
                let meta_far_contents =
                    std::fs::read(&blob.source_path).with_context(|| format!("reading {}", blob.source_path))?;
                let PackagedMetaFar {
                    abi_revision,
                    name,
                    meta_contents,
    ...
    fn write_contents_to_file<C: AsRef<[u8]>>(
        ...
        std::fs::write(&file_path, contents).context(format!("writing contents to file: {}", file_path.display()))?;
        Ok(file_path)
    }
    ...
    pub fn add_subpackage(
        ...
        self.subpackages
            .insert(url.clone(), (package_hash, package_manifest_path));
        Ok(())
    }
    ...
    pub fn api_level(&mut self, api_level: u64) -> Result<()> {
        for v in version_history::version_history().unwrap() {
    ...
    pub fn build(self, gendir: impl AsRef<Path>, metafar_path: impl AsRef<Path>) -> Result<PackageManifest> {
        ...
        let abi_revision = abi_revision.unwrap_or(version_history::latest_sdk_version().abi_revision);

        let abi_revision_file = Self::write_contents_to_file(gendir, ABI_REVISION_FILE_PATH, abi_revision.as_bytes())
        ...
        far_contents.insert(
            ...
            abi_revision_file
                .path_to_string()
                .with_context(|| format!("Adding the {ABI_REVISION_FILE_PATH} file to the package"))?,
        );
        ...
        if !subpackages.is_empty() {
            far_contents.insert(
                ...
                create_meta_subpackages_file(gendir, subpackages.clone())
                    .with_context(|| format!("Adding the {} file to the package", MetaSubpackages::PATH))?,
            );
        }

        let package_build_manifest = PackageBuildManifest::from_external_and_far_contents(blobs, far_contents)
        ...
        Ok(if let Some(manifest_path) = manifest_path {
            if let RelativeTo::File = blob_sources_relative {
                ...
                copy.write_with_relative_paths(&manifest_path).with_context(|| {
                    format!("Failed to create package manifest with relative paths at: {manifest_path}")
                })?;
                ...
                serde_json::ser::to_writer(BufWriter::new(&mut tmp), &package_manifest)
                    .with_context(|| format!("writing package manifest to {}", tmp.path().display()))?;
...
impl PackagedMetaFar {
    fn parse(bytes: &[u8]) -> Result<Self> {
        let mut meta_far = meshx_archive::Utf8Reader::new(Cursor::new(bytes)).context("reading FAR")?;
        ...
        for path in meta_paths {
            ...
            if path == MetaContents::PATH {
                ...
                let mp = MetaPackage::deserialize(Cursor::new(&contents)).context("deserializing meta/package")?;
        ...
        let abi_revision = abi_revision.ok_or_else(|| anyhow!("did not find {}", ABI_REVISION_FILE_PATH))?;
...
fn create_meta_subpackages_file(
    ...
    let meta_subpackages =
        MetaSubpackages::from_iter(subpackages.into_iter().map(|(name, (merkle, _))| (name, merkle)));
...
mod tests {
    use {
        super::*,
        camino::Utf8Path,
        meshx_merkle::MerkleTreeBuilder,
    ...
    fn test_builder() {
        ...
        let subpackage_hash = Hash::from([0; meshx_hash::HASH_SIZE]);
        ...
        builder
            ...
            .unwrap();
        builder
            .add_file_to_far("meta/some/file", far_source_file_path.path().path_to_string().unwrap())
            .unwrap();
        builder
            .add_subpackage(
                &subpackage_url,
        ...
        let subpackage_info = subpackages
            .iter()
            .find(|info| info.name == "subpackage0")
            .unwrap()
            .clone();
        ...
        let mut far_reader = meshx_archive::Utf8Reader::new(&mut metafar).unwrap();
        ...
        assert_eq!(abi_revision, version_history::latest_sdk_version().abi_revision.0);
    }
    ...
    fn test_from_manifest() {
        ...
        let fake_abi_revision = version_history::latest_sdk_version().abi_revision.0 + 1;
        ...
        let first_subpackage_hash = Hash::from([0; meshx_hash::HASH_SIZE]);
        ...
        // Build the package
        let first_manifest = first_builder
            .build(&first_outdir, first_outdir.path().join("meta.far"))
            .unwrap();
        assert_eq!(
        ...
        let far_reader = meshx_archive::Utf8Reader::new(&mut metafar).unwrap();
        let first_paths_in_far = far_reader.list().map(|e| e.path().to_string()).collect::<Vec<_>>();

        // Re-parse the package into a builder for further modification
        let second_outdir = TempDir::new().unwrap();
        let mut second_builder = PackageBuilder::from_manifest(first_manifest.clone(), second_outdir.path()).unwrap();
        ...
        second_builder
            .add_file_as_blob("some/other/blob", second_blob_source_file_path.path().to_string_lossy())
            .unwrap();
        ...
        for blob_info in second_manifest.blobs() {
            match &*blob_info.path {
                PackageManifest::META_FAR_BLOB_PATH => {
                    ...
                    let mut metafar = std::fs::File::open(&blob_info.source_path).unwrap();
                    let mut far_reader = meshx_archive::Utf8Reader::new(&mut metafar).unwrap();
                    let paths_in_far = far_reader.list().map(|e| e.path().to_string()).collect::<Vec<_>>();
                    assert_eq!(
                        paths_in_far.len(),
                    ...
                    for far_path in paths_in_far {
                        ...
                        match &*far_path {
                            ...
                            MetaSubpackages::PATH => {
                                let ms = MetaSubpackages::deserialize(Cursor::new(&far_bytes)).unwrap();
    ...
    fn test_build_rejects_abi_revision() {
        let mut builder = PackageBuilder::new("some_pkg_name");
        assert!(builder
            .add_file_to_far("meta/fuchsia.abi/abi-revision", "some/src/file")
            .is_err());
        assert!(builder
    ...
    fn test_builder_add_subpackages() {
        ...
        let pkg1_hash = Hash::from([0; meshx_hash::HASH_SIZE]);
        ...
        let pkg2_hash = Hash::from([1; meshx_hash::HASH_SIZE]);
        let pkg2_package_manifest_path = outdir.path().join("path2/package_manifest.json");

        builder
            .add_subpackage(&pkg1_url, pkg1_hash, pkg1_package_manifest_path)
            .unwrap();
        builder
            .add_subpackage(&pkg2_url, pkg2_hash, pkg2_package_manifest_path)
            .unwrap();
    ...
    fn test_builder_rejects_subpackages_collisions() {
        ...
        let hash1 = Hash::from([0; meshx_hash::HASH_SIZE]);
        let package_manifest_path1 = PathBuf::from("path1/package_manifest.json");
        let hash2 = Hash::from([0; meshx_hash::HASH_SIZE]);
