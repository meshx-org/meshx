...
impl PackageBuildManifest {
    ...
    pub fn from_external_and_far_contents(
        ...
        for (resource_path, _) in external_contents.iter().chain(far_contents.iter()) {
            validate_resource_path(resource_path).map_err(|e| PackageBuildManifestError::ResourcePath {
                cause: e,
                path: resource_path.to_string(),
            })?;
        }
        let external_paths = external_contents
            .keys()
        ...
        Ok(PackageBuildManifest(VersionedPackageBuildManifest::Version1(
            PackageBuildManifestV1 {
                external_contents,
                far_contents,
            },
        )))
...
mod tests {
    ...
    use meshx_url::errors::ResourcePathError::PathStartsWithSlash;
    use maplit::btreemap;
    use proptest::prelude::*;
    use serde_json::json;
    use std::fs::create_dir;

    fn from_json_value(value: serde_json::Value) -> Result<PackageBuildManifest, PackageBuildManifestError> {
