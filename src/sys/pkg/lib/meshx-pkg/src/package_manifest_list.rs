...
impl PackageManifestList {
    ...
    pub fn from_reader(manifest_list_path: &Utf8Path, mut reader: impl std::io::Read) -> anyhow::Result<Self> {
        ...
        let versioned_list = match serde_json::from_slice(bytes.as_slice())? {
            VersionedPackageManifestList::Version1(manifest_list) => {
                VersionedPackageManifestList::Version1(manifest_list.resolve_source_paths(manifest_list_path)?)
            }
        };
    ...
    pub fn write_with_relative_paths(self, path: &Utf8Path) -> anyhow::Result<Self> {
        let versioned = match &self.0 {
            VersionedPackageManifestList::Version1(package_manifest_list) => {
                VersionedPackageManifestList::Version1(package_manifest_list.clone().write_with_relative_paths(path)?)
            }
        };
    ...
    pub fn write_with_relative_paths(self, manifest_list_path: &Utf8Path) -> anyhow::Result<Self> {
        ...
        let manifest_list = if let RelativeTo::WorkingDir = &self.paths_relative {
            let manifests = self
                .manifests
                .into_iter()
                .map(|manifest_path| path_relative_from_file(manifest_path, manifest_list_path).unwrap())
                .collect::<Vec<_>>();
            Self {
                manifests,
    ...
    pub fn resolve_source_paths(self, manifest_list_path: &Utf8Path) -> anyhow::Result<Self> {
        if let RelativeTo::File = self.paths_relative {
            let manifests = self
                .manifests
                .into_iter()
                .map(|manifest_path| resolve_path_from_file(manifest_path, manifest_list_path).unwrap())
                .collect::<Vec<_>>();
...
impl From<PackageManifestList> for Vec<Utf8PathBuf> {
    fn from(package_manifest_list: PackageManifestList) -> Self {
        match package_manifest_list.0 {
            VersionedPackageManifestList::Version1(package_manifest_list_v1) => package_manifest_list_v1.manifests,
        }
    }
}
...
mod tests {
    ...
    fn test_iter_from_reader_json_v1() {
        ...
        let package_manifest_list =
            PackageManifestList::from_reader(&manifest_path, File::open(&manifest_path).unwrap()).unwrap();
        assert_eq!(
            PackageManifestList(VersionedPackageManifestList::Version1(PackageManifestListV1 {
        ...
        assert_eq!(
            PackageManifestList(VersionedPackageManifestList::Version1(PackageManifestListV1 {
                ...
                manifests: vec![
                    ...
                    manifest_dir.join("obj/src/sys/component_index/component_index/package_manifest.json"),
                ]
            })),
            package_manifest_list
        );
    ...
    fn test_write_package_manifest_list_making_paths_relative() {
        ...
        let result_manifest_list = package_manifest_list.write_with_relative_paths(&manifest_path).unwrap();
    ...
    fn test_write_package_manifest_list_already_relative() {
        ...
        let result_manifest_list = package_manifest_list
            .clone()
            .write_with_relative_paths(&manifest_path)
            .unwrap();
