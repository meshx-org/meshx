...
import { fx_object_wait_async, fx_port_create } from "@meshx-org/fiber-sys"
import {
    u64,
    FX_HANDLE_INVALID,
    fx_port_packet_t,
...
class Executor {
    ...
    constructor() {
        ...
        const status = fx_port_create(this.port)

        if (!this.port.value) throw new Error("failed to init signaling port status=" + status)

        const packet_out = new Ref<fx_port_packet_t>(undefined as any);
        
        self.fiber.sys_port_wait(this.port.value, FX_TIME_INFINITE, packet_out).then((status) => {
            packet_out.value.key
            packet_out.value.packet_type
            packet_out.value.union
            packet_out.value.status
        })
    }
...
export class HandleWaiter implements PacketReceiver {
    ...
    public cancel(): void {
        if (this.handle) {
            ...
            // TODO: this.handle.releaseWaiter(this)
            this.handle = null
        }
    }
}
