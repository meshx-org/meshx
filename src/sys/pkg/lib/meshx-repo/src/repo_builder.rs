...
use {
    ...
    meshx_merkle::Hash,
    meshx_pkg::{BlobInfo, PackageManifest, PackageManifestList, PackagePath, SubpackageInfo},
    ...
    tempfile::TempDir,
    tuf::{
        crypto::HashAlgorithm, metadata::TargetPath, pouf::Pouf1, repo_builder::RepoBuilder as TufRepoBuilder, Database,
    },
};
...
{
    pub fn from_client(client: &'a RepoClient<R>, repo_keys: &'a RepoKeys) -> RepoBuilder<'a, &'a R> {
    ...
    R: RepoStorageProvider,
{
    ...
    fn new(repo: R, trusted_repo_keys: &'a RepoKeys, database: Option<&'a Database<Pouf1>>) -> RepoBuilder<'a, R> {
        RepoBuilder {
            repo,
    ...
    pub async fn add_packages(mut self, paths: impl Iterator<Item = Utf8PathBuf>) -> Result<RepoBuilder<'a, R>> {
        for path in paths {
    ...
    pub async fn add_package_archive(self, path: Utf8PathBuf) -> Result<RepoBuilder<'a, R>> {
        ...
        let manifest = PackageManifest::from_archive(path.as_std_path(), blobs_out.path(), manifest_out.path())
        ...
        self.stage_named_package(
            ...
            ToBeStagedPackage {
                ...
                kind: ToBeStagedPackageKind::Archive {
                    _archive_out: blobs_out,
                    manifest,
                },
            },
        )
    ...
    async fn stage_package(
        ...
        if self.staged_packages.contains_key(&package_hash) || to_be_staged_packages.contains_key(&package_hash) {
            return Ok(true);
        }
        ...
        let blobs = {
            ...
            let stream = futures::stream::iter(unique_package_blobs.iter())
                .filter_map(|blob| async move {
                    if self.staged_blobs.contains_key(&blob.merkle) || to_be_staged_blobs.contains_key(&blob.merkle) {
                        None
                    } else {
                        Some(async move {
            ...
            while let Some((blob, result)) = stream.next().await {
                match result {
                    Ok(metadata) => {
                        ...
                        if let Some(first_merkle) = seen_links.insert((metadata.dev(), metadata.ino()), blob.merkle) {
                    ...
                    Err(err) => {
                        if self.ignore_missing_packages && err.kind() == std::io::ErrorKind::NotFound {
                            ...
                            return Err(err).with_context(|| format!("checking if {} exists", blob.source_path));
                        }
                    }
                }
            }
        ...
        for subpackage in package.manifest().subpackages() {
                ...
                && !to_be_staged_packages.contains_key(&subpackage.merkle)
            {
                ...
                if !self
                    .stage_subpackage(manifest_path, to_be_staged_packages, to_be_staged_blobs, deps)
                    .await?
                {
                    return Ok(false);
                }
    ...
    pub async fn add_package_list(mut self, path: Utf8PathBuf) -> Result<RepoBuilder<'a, R>> {
        let contents = fs::read(path.as_std_path()).with_context(|| format!("reading package manifest list {path}"))?;
    ...
    pub async fn add_package_lists(mut self, paths: impl Iterator<Item = Utf8PathBuf>) -> Result<RepoBuilder<'a, R>> {
        for path in paths {
    ...
    pub async fn commit(self) -> Result<(HashSet<Utf8PathBuf>, HashMap<Hash, BlobInfo>)> {
        ...
        let () = futures::stream::iter(&self.staged_blobs)
            .map(Ok)
            .try_for_each_concurrent(std::thread::available_parallelism()?.get(), |(blob_hash, blob)| {
                self.repo
                    .store_blob(blob_hash, blob.size, Utf8Path::new(&blob.source_path))
            })
            .await?;
...
fn check_manifests_are_equivalent(old_package: &ToBeStagedPackage, new_package: &ToBeStagedPackage) -> Result<()> {
    ...
    let old_manifest_path = old_package
        .manifest_path
        .as_ref()
        .map(|path| path.as_str())
        .unwrap_or("<generated>");
    let new_manifest_path = new_package
        .manifest_path
        .as_ref()
        .map(|path| path.as_str())
        .unwrap_or("<generated>");
    ...
    fn manifest_contents(manifest: &PackageManifest) -> Result<BTreeMap<String, BlobEntry>> {
        ...
        for blob in manifest.blobs() {
            if blob.path == "meta/" {
                let file = std::fs::File::open(&blob.source_path).with_context(|| format!("reading {}", blob.path))?;
                let mut far = meshx_archive::Utf8Reader::new(file)?;

                let far_entries = far.list().map(|entry| entry.path().to_owned()).collect::<Vec<_>>();
    ...
    if let (Ok(old_contents), Ok(mut new_contents)) = (
        manifest_contents(old_package.manifest()),
        manifest_contents(new_package.manifest()),
    ) {
    ...
    fn manifest_subpackages(manifest: &PackageManifest) -> Result<BTreeMap<String, SubpackageInfo>> {
        let mut entries = BTreeMap::new();
    ...
    if let (Ok(old_subpackages), Ok(mut new_subpackages)) = (
        manifest_subpackages(old_package.manifest()),
        manifest_subpackages(new_package.manifest()),
    ) {
...
mod tests {
    ...
    async fn test_create_and_update_repo() {
        ...
        serde_json::to_writer(std::fs::File::create(&pkg1_manifest_path).unwrap(), &pkg1_manifest).unwrap();
        ...
        serde_json::to_writer(std::fs::File::create(&pkg2_manifest_path).unwrap(), &pkg2_manifest).unwrap();
    ...
    async fn test_create_and_update_repo_with_subpackages() {
        ...
        let (anonsubpkg_meta_far_path, anonsubpkg_manifest) =
            test_utils::make_package_manifest(ANONYMOUS_SUBPACKAGE, anonsubpkg_dir.as_std_path(), Vec::new());
        ...
        expected_deps.extend(
            anonsubpkg_manifest
                .blobs()
                .iter()
                .map(|blob| Utf8PathBuf::from(&blob.source_path)),
        );
        ...
        assert_eq!(
            ...
            BTreeMap::from([
                ...
                (
                    test_utils::ANONSUBPKG_BIN_HASH.into(),
                    b"binary anonymous_subpackage".to_vec()
                ),
                (
                    test_utils::ANONSUBPKG_LIB_HASH.into(),
                    b"lib anonymous_subpackage".to_vec()
                ),
                (
                    test_utils::NAMEDSUBPKG_HASH.into(),
                    namedsubpkg_meta_far_contents.clone()
                ),
                (
                    test_utils::NAMEDSUBPKG_BIN_HASH.into(),
                    b"binary named_subpackage".to_vec()
                ),
                (
                    test_utils::NAMEDSUBPKG_LIB_HASH.into(),
                    b"lib named_subpackage".to_vec()
                ),
    ...
    async fn test_error_if_package_manifest_is_missing() {
        ...
        assert_matches!(
            RepoBuilder::create(&repo, &repo_keys)
                .add_package(dir.join("does-not-exist"))
                .await,
            Err(_)
        );
    }
    ...
    async fn test_do_not_stage_blobs_if_ignore_missing_files_and_package_files_are_missing() {
        ...
        let (_, pkg_manifest) = test_utils::make_package_manifest("package", dir.as_std_path(), vec![]);
    ...
    async fn test_error_if_subpackage_manifest_is_missing() {
        ...
        serde_json::to_writer(std::fs::File::create(&subpkg_manifest_path).unwrap(), &subpkg_manifest).unwrap();
    ...
    async fn test_refresh_metadata_with_no_keys() {
        ...
        let res = RepoBuilder::from_database(repo_client.remote_repo(), &repo_keys, repo_client.database())
            .refresh_metadata(true)
            .commit()
            .await;
    ...
    async fn test_refresh_metadata_with_root_metadata() {
        ...
        let repo_keys = RepoKeys::from_dir(&full_repo_path.join("keys").into_std_path_buf()).unwrap();
        RepoBuilder::from_database(repo_client.remote_repo(), &repo_keys, repo_client.database())
            .refresh_metadata(true)
            .commit()
            .await
    ...
    async fn test_inherit_from_trusted_targets() {
        ...
        let (_, pkg3_manifest) = test_utils::make_package_manifest("package3", pkg3_dir.as_std_path(), Vec::new());
        let pkg3_manifest_path = pkg3_dir.join("package3.manifest");
        serde_json::to_writer(std::fs::File::create(&pkg3_manifest_path).unwrap(), &pkg3_manifest).unwrap();
        ...
        let (_, pkg4_manifest) = test_utils::make_package_manifest("package4", pkg4_dir.as_std_path(), Vec::new());
        let pkg4_manifest_path = pkg4_dir.join("package4.manifest");
        serde_json::to_writer(std::fs::File::create(&pkg4_manifest_path).unwrap(), &pkg4_manifest).unwrap();
    ...
    async fn test_key_rotation() {
        ...
        RepoBuilder::from_database(repo_client.remote_repo(), &repo_trusted_keys, repo_client.database())
    ...
    async fn test_conflicting_package_manifests_errors_out() {
        ...
        let pkg1_dir = dir.join("package1");
        let (_, pkg1_manifest) = test_utils::make_package_manifest("package1", pkg1_dir.as_std_path(), Vec::new());
        let pkg1_manifest_path = pkg1_dir.join("package1.manifest");
        serde_json::to_writer(std::fs::File::create(&pkg1_manifest_path).unwrap(), &pkg1_manifest).unwrap();
        ...
        let pkg2_manifest = PackageBuilder::new("package1")
    ...
    async fn test_stage_package_overwrites_corrupted_blob_in_blob_store() {
        ...
        let hash = meshx_merkle::from_slice(contents).root();
