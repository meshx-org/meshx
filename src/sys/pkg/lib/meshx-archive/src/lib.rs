...
//! # Reading, Writing and Listing MeshX Archives (MARC) Data
//!
//!
//! # Example
//!
...
//! meshx_archive::write(&mut result, path_content_map).unwrap();
//! let result = &result[..];
//!
//! let reader = meshx_archive::Reader::new(Cursor::new(result)).unwrap();
...
fn validate_directory_entries_and_paths(
    ...
    for (i, entry) in directory_entries.iter().enumerate() {
        ...
        let () = validate_content_chunk(entry, previous_entry, name, stream_len, end_of_last_non_content_chunk)?;
...
fn list<'a>(directory_entries: &'a [DirectoryEntry], path_data: &'a [u8]) -> impl ExactSizeIterator<Item = Entry<'a>> {
...
fn find_directory_entry<'a>(
    ...
    // MARC spec requires, and [Async]Reader::new enforces, that directory entries are sorted by
    // path data
    ...
    Ok(directory_entries
        .get(i)
        .expect("binary_search on success returns in-bounds index"))
}
...
fn next_multiple_of(unrounded_value: u64, multiple: u64) -> u64 {
    let rem = unrounded_value
        .checked_rem(multiple)
        .expect("never called with multiple = 0");
    if rem > 0 {
...
pub(crate) mod tests {
    ...
    fn test_serialize_deserialize_index() {
        ...
        let () = target
            .get_ref()
            .as_slice()
            .read_exact(decoded_index.as_bytes_mut())
            .unwrap();
    ...
    fn test_serialize_deserialize_index_entry() {
        ...
        let () = target
            .get_ref()
            .as_slice()
            .read_exact(decoded_index_entry.as_bytes_mut())
            .unwrap();
    ...
    fn test_serialize_deserialize_directory_entry() {
        ...
        let () = target
            .get_ref()
            .as_slice()
            .read_exact(decoded_directory_entry.as_bytes_mut())
            .unwrap();
