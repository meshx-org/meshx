...
impl OnSignalsReceiver {
    fn get_signals(&self, cx: &mut Context<'_>) -> Poll<fx::Signals> {
        ...
        if signals == 0 {
            ...
            Poll::Ready(fx::Signals::from_bits_truncate(signals as u32))
        }
    }

    fn set_signals(&self, signals: fx::Signals) {
...
impl PacketReceiver for OnSignalsReceiver {
    fn receive_packet(&self, packet: fx::Packet) {
        let observed = if let fx::PacketContents::SignalOne(p) = packet.contents() {
            p.observed()
        } else {
            return;
        };
...
pub struct OnSignals<'a> {
    state: Result<ReceiverRegistration<OnSignalsReceiver>, fx::Status>,
    handle: Option<(fx::HandleRef<'a>, EHandle)>,
}

impl<'a> OnSignals<'a> {
    /// Creates a new `OnSignals` object which will receive notifications when
    /// any signals in `signals` occur on `handle`.
    pub fn new<T: AsHandleRef>(handle: &'a T, signals: fx::Signals) -> Self {
    ...
    pub fn from_ref(handle: fx::HandleRef<'a>, signals: fx::Signals) -> Self {
    ...
    pub fn extend_lifetime(mut self) -> OnSignals<'static> {
        OnSignals { state: std::mem::replace(&mut self.state, Err(fx::Status::OK)), handle: None }
    }
}
...
impl Future for OnSignals<'_> {
    type Output = Result<fx::Signals, fx::Status>;
    fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        let reg = self.state.as_mut().map_err(|e| mem::replace(e, fx::Status::OK))?;
...
mod test {
    ...
    fn wait_for_event() -> Result<(), fx::Status> {
        ...
        // no longer pending
        event.signal_handle(fx::Signals::NONE, fx::Signals::EVENT_SIGNALED)?;
        deliver_events();
        assert_eq!(waker_count, 1);
        assert_eq!(signals.poll_unpin(cx), Poll::Ready(Ok(fx::Signals::EVENT_SIGNALED)));
        Ok(())
    }

    #[test]
    fn drop_before_event() -> Result<(), fx::Status> {
        let _exec = crate::TestExecutor::new();
        let ehandle = EHandle::local();
        let event = fx::Event::create();
