...
mod tests {
    use {
        super::*, crate::errors::PackagePathSegmentError, assert_matches::assert_matches, std::convert::TryFrom as _,
    };
    ...
    fn new_with_path_ok() {
        ...
        let url = UnpinnedAbsolutePackageUrl::new_with_path(repo.clone(), "/name/variant".into()).unwrap();
        assert_eq!(url.name().as_ref(), "name");
    ...
    fn parse_err() {
        for (url, err) in [
            ...
            (
                "fuchsia-pkg://example.org/name/variant/extra",
    ...
    #[test]
    fn clear_variant() {
        let mut url = UnpinnedAbsolutePackageUrl::parse("fuchsia-pkg://example.org/name/variant").unwrap();
        url.clear_variant();
        assert_eq!(url.variant(), None);
