...
struct Crates {
    #[darling(default = "Self::default_controlplane_core")]
    controlplane_core: Path,
    #[darling(default = "Self::default_controlplane_api")]
    controlplane_api: Path,
...
impl Crates {
    fn default_controlplane_core() -> Path {
        parse_quote! { ::controlplane_core } // by default must work well with people using facade crate
    }

    fn default_controlplane_api() -> Path {
        parse_quote! { ::controlplane_api }
    }
...
pub(crate) fn derive(input: proc_macro2::TokenStream) -> proc_macro2::TokenStream {
    ...
    match derive_input.data {
        ...
        _ => {
            return syn::Error::new_spanned(&derive_input.ident, r#"Unions can not #[derive(CustomResource)]"#)
                .to_compile_error()
        }
    }
    ...
    let StatusInformation {
        ...
        impl_hasstatus,
    } = process_status(&rootident, &status, &visibility, &controlplane_core);
    ...
    let docstr = doc.unwrap_or_else(|| format!(" Auto-generated derived type for {ident} via `CustomResource`"));
    ...
    let root_obj = quote! {
        ...
        #visibility struct #rootident {
            #schemars_skip
            #visibility metadata: #controlplane_api::apimachinery::apis::meta::v1::ObjectMeta,
        ...
        impl #rootident {
            ...
            pub fn new(name: &str, spec: #ident) -> Self {
                Self {
                    metadata: #controlplane_api::apimachinery::apis::meta::v1::ObjectMeta {
        ...
        impl #serde::Serialize for #rootident {
            fn serialize<S: #serde::Serializer>(&self, ser: S) -> #std::result::Result<S::Ok, S::Error> {
                ...
                obj.serialize_field("apiVersion", &<#rootident as #controlplane_core::Resource>::api_version(&()))?;
                obj.serialize_field("kind", &<#rootident as #controlplane_core::Resource>::kind(&()))?;
    ...
    let (scope, scope_quote) = if namespaced {
        ("Namespaced", quote! { #controlplane_core::NamespaceResourceScope })
    } else {
        ("Cluster", quote! { #controlplane_core::ClusterResourceScope })
    };

    let api_ver = format!("{group}/{version}");
    let impl_resource = quote! {
        impl #controlplane_core::Resource for #rootident {
            type DynamicType = ();
            ...
            fn meta(&self) -> &#controlplane_api::apimachinery::apis::meta::v1::ObjectMeta {
                &self.metadata
            }

            fn meta_mut(&mut self) -> &mut #controlplane_api::apimachinery::apis::meta::v1::ObjectMeta {
                &mut self.metadata
            }
        }
    };
    ...
    let impl_default = if has_default {
        quote! {
            impl Default for #rootident {
                fn default() -> Self {
                    Self {
                        metadata: #controlplane_api::apimachinery::apis::meta::v1::ObjectMeta::default(),
    ...
    let apiext = quote! {
        #controlplane_api::apiextensions::v1
    };
    let extver = quote! {
        #controlplane_core::crd::v1
    };

    let shortnames_slice = {
        let names = shortnames.iter().map(|name| quote! { #name, }).collect::<TokenStream>();
        quote! { &[#names] }
    };
    ...
    let schemagen = if schema_mode.use_in_crd() {
        quote! {
            ...
            let gen = #schemars::gen::SchemaSettings::openapi3()
                ...
                .with_visitor(#controlplane_core::schema::StructuralSchemaRewriter)
                .into_generator();
        ...
        quote! {
            let schema: Option<#controlplane_api::apiextensions::v1::JSONSchemaProps> = None;
        }
    };
    ...
    let impl_crd = quote! {
        impl #extver::CustomResourceExt for #rootident {
            ...
            fn api_resource() -> #controlplane_core::dynamic::ApiResource {
                #controlplane_core::dynamic::ApiResource::erase::<Self>(&())
            }
    ...
    let impl_hasspec = generate_hasspec(&ident, &rootident, &controlplane_core);
...
/// This generates the code for the `#controlplane_core::object::HasSpec` trait implementation.
///
...
/// * `controlplane_core`: The path stream for the analagous kube::core import location from users POV
fn generate_hasspec(spec_ident: &Ident, root_ident: &Ident, controlplane_core: &Path) -> TokenStream {
    quote! {
        impl #controlplane_core::object::HasSpec for #root_ident {
    ...
    if let Some(status_name) = &status {
        ...
        StatusInformation {
            ...
            impl_hasstatus: quote! {
                impl #controlplane_core::object::HasStatus for #root_ident {
...
mod tests {
    ...
    fn test_derive_crd() {
        ...
        let path = env::current_dir().unwrap().join("tests").join("crd_schema_test.rs");
        let file = fs::File::open(path).unwrap();
        runtime_macros::emulate_derive_macro_expansion(file, &[("CustomResource", derive)]).unwrap();
    }
}
