//! Watches a MeshX Resource for changes, with error recovery
//!
//! See [`watcher`] for the primary entry point.

use async_trait::async_trait;
use backoff::{backoff::Backoff, ExponentialBackoff};
use controlplane_client::error::ErrorResponse;
use controlplane_client::{
    api::{ListParams, Resource, VersionMatch, WatchEvent, WatchParams},
    core::ObjectList,
    Api, Error as ClientErr, ResourceExt,
};
...
enum State<K> {
    ...
    InitialWatch {
        ...
        stream: BoxStream<'static, Result<WatchEvent<K>, controlplane_client::Error>>,
    },
...
trait ApiMode {
    ...
    async fn list(&self, lp: &ListParams) -> Result<ObjectList<Self::Value>, controlplane_client::Error>;
    async fn watch(
        &self,
        wp: &WatchParams,
        version: &str,
    ) -> Result<
        BoxStream<'static, Result<WatchEvent<Self::Value>, controlplane_client::Error>>,
        controlplane_client::Error,
    >;
}
...
impl Config {
    ...
    fn to_list_params(&self) -> ListParams {
        ...
        ListParams {
            ...
            timeout: self.timeout.map(|v| Duration::from_secs(v as u64)),
            version_match,
            resource_version,
    ...
    fn to_watch_params(&self) -> WatchParams {
        WatchParams {
            ...
            send_initial_events: Some(self.initial_list_strategy == InitialListStrategy::StreamingList),
        }
    }
}
...
async fn step_trampolined<A>(
    ...
    wc: &Config,
    state: State<A::Value>,
) -> (Option<Result<Event<A::Value>, Error>>, State<A::Value>)
where
    A: ApiMode,
    A::Value: Resource + 'static,
{
    match state {
        State::Empty => match wc.initial_list_strategy {
            InitialListStrategy::ListWatch => (Some(Ok(Event::Init)), State::InitPage { continue_token: None }),
        ...
        State::InitPageDone { resource_version } => (Some(Ok(Event::InitDone)), State::InitListed { resource_version }),
        ...
        State::InitListed { resource_version } => match api.watch(&wc.to_watch_params(), &resource_version).await {
            Ok(stream) => (
                None,
                State::Watching {
                    resource_version,
...
) -> (Result<Event<A::Value>, Error>, State<A::Value>)
where
    A: ApiMode,
    A::Value: Resource + 'static,
{
