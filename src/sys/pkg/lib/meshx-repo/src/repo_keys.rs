...
use {
    ...
    ed25519_dalek::{SecretKey, Signature, Signer, SigningKey, Verifier, PUBLIC_KEY_LENGTH, SECRET_KEY_LENGTH},
...
impl fmt::Debug for RepoKeys {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("RepoKeys")
            .field(
                "root_keys",
                &self.root_keys.iter().map(|key| key.public()).collect::<Vec<_>>(),
            )
            .field(
                "targets_keys",
...
impl RepoKeys {
    ...
    pub fn generate(dir: &Path) -> Result<Self, ParseError> {
        ...
        fn generate_tuf_keyval(keytype: &KeyType) -> Result<KeyVal, ParseError> {
            match keytype {
                &KeyType::Ed25519 => {
                    ...
                    public_key_bytes[..].copy_from_slice(&signing_key.verifying_key().to_bytes());
                ...
                _ => Err(ParseError::UnsupportedKeyTypeGeneration {
        ...
        fn generate_rolekey_collection(
            ...
            match keytype {
                &KeyType::Ed25519 => {
                    keys.push(Box::new(Ed25519PrivateKey::from_ed25519(&role_key.keyval.private)?) as Box<_>);
                }
                _ => {
        ...
        fn write_rolekeys(dir: &Path, rolekeys_filename: &str, rolekeys: RoleKeys) -> Result<(), ParseError> {
        ...
        Ok(Self {
            ...
            timestamp_keys: generate_rolekey_collection(DEFAULT_KEYTYPE_GENERATION, &timestamp_key)?,
        })
    }
    ...
    fn test_from_dir_generated_keys() {
        macro_rules! assert_repo_keys {
            ($generated:expr, $parsed:expr) => {
                let generated: Vec<&PublicKey> = $generated.iter().map(|key| key.public()).collect::<_>();
