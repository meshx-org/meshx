...
impl RepositoryServer {
    ...
    pub fn builder(addr: SocketAddr, repo_manager: Arc<RepositoryManager>) -> RepositoryServerBuilder {
...
impl RepositoryServerBuilder {
    ...
    pub async fn start(
        ...
        let server_fut = run_server(listener, rx_conns, rx_stop_server, Arc::clone(&self.repo_manager));
    ...
    let resource = match resource_path {
        "auto" => {
            if repo.read().await.supports_watch() {
                return handle_auto(executor, server_stopped, repo_name, repo, sse_response_creators).await;
            } else {
        ...
        "repo.config" => {
            ...
            let mirror_url = match Uri::try_from(format!("http://{host_ssh_connection}/{repo_name}")) {
                Ok(m) => m,
                Err(e) => {
...
async fn handle_auto(
    ...
    let (response_creator, sender) = {
        ...
        /*if let Some(response_creator) = sse_response_creators.get(repo_name) {
            ...
            let (response_creator, sender) = SseResponseCreator::with_additional_buffer_size(AUTO_BUFFER_SIZE);
...
// any errors.
async fn read_timestamp_version(repo: Arc<AsyncRwLock<RepoClient<Box<dyn RepoProvider>>>>) -> Option<u32> {
...
impl tokio::io::AsyncRead for ConnectionStream {
    fn poll_read(
        ...
        match &mut *self {
            ...
            ConnectionStream::Socket(t, _) => futures::AsyncRead::poll_read(Pin::new(t), cx, buf.initialize_unfilled()),
        }
        .map_ok(|sz| {
            buf.advance(sz);
        })
...
mod tests {
    ...
    ///
    /// This takes a `client`, which allows hyper to do connection pooling.
    async fn get_bytes(client: &HttpClient, url: impl AsRef<str> + std::fmt::Debug) -> Result<Bytes> {
    ...
    async fn verify_repo_config(client: &HttpClient, devhost: &str, server_url: &str) {
        let config: RepositoryConfig = serde_json::from_slice(
            &get_bytes(client, &format!("{server_url}/{devhost}/repo.config"))
                .await
                .unwrap(),
        )
        .unwrap();

        let expected = RepositoryConfigBuilder::new(RepositoryUrl::parse(&format!("fuchsia-pkg://{devhost}")).unwrap())
            .add_mirror(
        ...
        R: Future<Output = ()>,
    {
        ...
        let (server_fut, _, server) = RepositoryServer::builder(addr, Arc::clone(&manager))
            .start()
            .await
            .unwrap();
    ...
    async fn test_get_repositories_from_empty_path() {
        ...
        run_test(manager, |server_url| async move {
            ...
            let middle =
                "<li><a href=\"devhost-0\">devhost-0</a></li><li><a href=\"devhost-1\">devhost-1</a></li>".to_string();
        ...
        run_test(manager, |server_url| async move {
            for (devhost, bodies) in &test_cases {
                for body in &bodies[..] {
                    ...
                    assert_eq!(
                        &body[1..=2],
                        get_bytes_range(client, &url, Some(1), Some(2), body.chars().count().try_into().unwrap(),)
                            .await
                            .unwrap()
                    );
                }
    ...
    async fn test_auto_inner() {
        ...
        write_file(
            ...
            serde_json::to_string(&SignedTimestamp {
                signed: TimestampFile { version: 1 },
            })
            .unwrap()
            .as_bytes(),
        );
